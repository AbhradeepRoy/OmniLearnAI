...
import { ChevronRight, ChevronLeft, Download, Layers, MessageSquare, BrainCircuit, Star, Flame, Trophy, Mic, Info, Loader2, Sparkles, Play } from 'lucide-react';
import Markdown from 'react-markdown';
import { generateSlides, translateText, generateQuiz } from '../../services/geminiService';
...
export default function SolutionRenderer({ analysis, onClose, onFollowUp, onNextChallenge, onAwardBadge }: SolutionRendererProps) {
  ...
  const [translatedTextContent, setTranslatedTextContent] = useState<string | null>(null);
  ...
  useEffect(() => {
    ...
    setError(null);
    setIsLoading(true);
    setTranslatedTextContent(null);
    
    async function loadContent() {
      try {
        let result = null;
        if (format === 'slides') {
          result = await generateSlides(analysis.concept, analysis.complexity);
        } else if (format === 'quiz') {
          result = await generateQuiz(analysis.concept, analysis.complexity);
          setQuizScore(0);
  ...
  const handleTranslateSpecific = async (targetLang: string) => {
    ...
    try {
      let originalText = "";
      if ((format === 'text' || format === 'voice') && content?.text) {
        originalText = content.text;
      } else if (format === 'slides' && content?.[currentSlide]) {
      ...
      if (format === 'text' || format === 'voice') {
        setContent({ ...content, text: translated });
      } else if (format === 'slides' && content?.[currentSlide]) {
        const newContent = [...content];
        const points = translated.split('\n').filter(p => p.trim()).map(p => p.replace(/^[•\-\d\.]\s*/, '').trim());
                ...
                >
                  <div className="flex-1 rounded-[32px] relative overflow-hidden bg-[#050505] text-white flex flex-col border border-white/5 shadow-2xl min-h-[450px]">
                    ...
                    <div className="flex-1 flex flex-col md:flex-row relative z-10 overflow-hidden">
                        ...
                        {content[currentSlide] && (
                          <div className="space-y-3 md:space-y-4">
                            <h3 className="text-lg md:text-2xl font-black text-white leading-tight">
                               <span className="text-brand-500 mr-2">#0{currentSlide + 1}</span> {content[currentSlide]?.title}
                            </h3>
                            <div className="space-y-2 md:space-y-2.5 max-w-2xl">
                              {Array.isArray(content[currentSlide].content) ? (
                                content[currentSlide].content.map((point: string, i: number) => (
                                  <motion.div 
                                    key={i} 
                                ...
                                <p className="text-xs md:text-base text-slate-300 leading-relaxed">{content[currentSlide]?.content}</p>
                              )}
                            </div>
                          </div>
                        )}
                      ...
                      {/* Image Content */}
                      {content?.[currentSlide]?.imageSearchTerm && (
                        <motion.div 
                        ...
                        >
                           <img 
                             src={`https://source.unsplash.com/featured/800x800?${encodeURIComponent(content[currentSlide]?.imageSearchTerm || analysis.concept)}`} 
                             alt={content[currentSlide]?.imageSearchTerm}
                             loading="lazy"
                             referrerPolicy="no-referrer"
                             className="w-full h-full object-cover opacity-90 hover:opacity-100 transition-opacity"
                             onError={(e) => {
                               (e.target as HTMLImageElement).src = `https://loremflickr.com/800/800/${encodeURIComponent(content[currentSlide]?.imageSearchTerm || analysis.concept)}?lock=${currentSlide}`;
                             }}
                           />
                  ...
                  <div className="flex items-center justify-between px-4 pb-2">
                     <div className="flex gap-2">
                        {Array.isArray(content) && content.map((_: any, i: number) => (
                          <button 
                            key={i} 
                     ...
                     <div className="flex gap-3 md:gap-4">
                        ...
                        <button 
                          disabled={Array.isArray(content) && currentSlide === content.length - 1}
                          onClick={() => setCurrentSlide(s => s + 1)}
                ...
                {!showResults ? (
                  ...
                  >
                    <div className="flex items-center justify-between">
                       <span className="px-4 py-1.5 bg-brand-50 dark:bg-brand-900/30 text-brand-600 dark:text-brand-400 rounded-full text-[10px] font-bold uppercase tracking-widest">
                          Question {currentQuestionIndex + 1} of {content.length}
                       </span>
                    ...
                    <h3 className="text-base md:text-lg font-black text-slate-800 dark:text-white leading-tight">
                      {content[currentQuestionIndex]?.question}
                    </h3>

                    <div className="grid grid-cols-1 gap-2">
                      {Array.isArray(content[currentQuestionIndex]?.options) && content[currentQuestionIndex].options.map((option: string, i: number) => (
                        <button
                          key={i}
                          disabled={selectedAnswer !== null}
                          onClick={() => {
                            setSelectedAnswer(i);
                            if (i === content[currentQuestionIndex].correctAnswer) {
                              setQuizScore(s => s + 1);
                            }
                          }}
                          className={`p-2 md:p-2.5 rounded-[10px] text-left text-[10px] md:text-[11px] font-semibold transition-all border-2 flex items-center justify-between group ${
                            selectedAnswer === null 
                              ...
                              : selectedAnswer === i 
                                ? i === content[currentQuestionIndex].correctAnswer 
                        ...
                        >
                          {option}
                          {selectedAnswer !== null && i === content[currentQuestionIndex].correctAnswer && (
                    ...
                    <AnimatePresence>
                      {selectedAnswer !== null && (
                        <motion.div 
                          ...
                          animate={{ opacity: 1, y: 0 }}
                          className={`p-3 md:p-4 rounded-xl ${selectedAnswer === content[currentQuestionIndex].correctAnswer ? 'bg-emerald-50 dark:bg-emerald-900/10 text-emerald-700 dark:text-emerald-300' : 'bg-slate-50 dark:bg-slate-900/50 text-slate-600 dark:text-slate-400'}`}
                        >
                          <p className="font-bold flex items-center gap-2 mb-1 text-[11px]">
                             <Info size={14} /> {selectedAnswer === content[currentQuestionIndex].correctAnswer ? 'Brilliant!' : 'Learning Opportunity'}
                          </p>
                          <p className="text-[10px] md:text-[10px] opacity-90">{content[currentQuestionIndex]?.explanation}</p>
                          
                          <button 
                            onClick={() => {
                              if (Array.isArray(content) && currentQuestionIndex < content.length - 1) {
                                setCurrentQuestionIndex(s => s + 1);
                                setSelectedAnswer(null);
                              } else {
                                if (quizScore === content.length && onAwardBadge) {
                                  onAwardBadge({
                          ...
                          >
                            {currentQuestionIndex < content.length - 1 ? 'Next Question' : 'View Results'}
                          </button>
                        </motion.div>
                      )}
                    </AnimatePresence>
                  ...
                  >
                    ...
                    <div className="text-5xl font-black text-brand-600">
                      {Array.isArray(content) && content.length > 0 ? Math.round((quizScore / content.length) * 100) : 0}%
                    </div>
                    <p className="text-slate-600 dark:text-slate-400 font-bold text-sm">
                       You got {quizScore} out of {Array.isArray(content) ? content.length : 0} questions correctly.
                    </p>
                ...
                >
                   ...
                   <div className="max-w-2xl text-center space-y-8">
                      ...
                      <div className="p-10 bg-white dark:bg-slate-800 rounded-[32px] border border-slate-100 dark:border-slate-700 italic text-xl md:text-2xl text-slate-600 dark:text-slate-300 leading-relaxed shadow-xl">
                         "{content?.text?.substring(0, 250) || "Preparing personalized audio explanation..."}..."
                      </div>
